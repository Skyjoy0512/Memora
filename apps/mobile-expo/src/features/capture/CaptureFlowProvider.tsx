...
export function CaptureFlowProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const isFinished =
      generationPhase === "completed" || generationPhase === "failed";
  ...
  function startGeneration(
    file: AudioFile,
    request: { name?: string; options: SummaryOptionsDTO },
  ) {
...
export function useCaptureFlow() {
  ...
  if (!context)
    throw new Error("useCaptureFlow must be used inside CaptureFlowProvider");
  return context;
}
...
function RecordingOverlay({
  ...
  const waveform = Array.from(
    { length: 18 },
    (_, index) => 6 + ((elapsedSeconds * 11 + index * 17) % 55),
  );
...
function GenerateOverlay({
  ...
  const [templateId, setTemplateId] = useState<SummaryOptionsDTO["templateId"]>(
    generateTemplates[0].id,
  );
        ...
        <View style={styles.generateModeRow}>
          <Pressable
            ...
            style={[
              ...
              genMode === "auto" && styles.generateModeCardActive,
            ]}
          >
            <Text
              style={[
                styles.generateModeTitle,
                genMode === "auto" && styles.generateModeTextActive,
              ]}
            >
              自動
            </Text>
          ...
          <Pressable
            ...
            style={[
              styles.generateModeCard,
              genMode === "custom" && styles.generateModeCardActive,
            ]}
          >
            <Text
              style={[
                styles.generateModeTitle,
                genMode === "custom" && styles.generateModeTextActive,
              ]}
            >
              テンプレート
            </Text>
          ...
          <View style={styles.generateTemplateRow}>
            {generateTemplates.map((item) => (
              <Pressable
                ...
                style={[
                  styles.generateChip,
                  templateId === item.id && styles.generateChipActive,
                ]}
              >
                <Text
                  style={[
                    styles.generateChipText,
                    templateId === item.id && styles.generateModeTextActive,
                  ]}
                >
                  {item.label}
                </Text>
  ...
  return (
    <SafeAreaView style={styles.generationScreen}>
      <View style={styles.generationContent}>
        {isComplete ? (
          <Ionicons
            color={phase === "completed" ? colors.success : colors.danger}
            name={phase === "completed" ? "checkmark-circle" : "alert-circle"}
            size={52}
          />
        ) : (
...
function DynamicIslandPill({
  ...
  if (showSnackbar) {
    return (
      ...
      >
        <Ionicons
          color={snackbarError ? colors.accent : colors.success}
  ...
  if (isRecordingActive) {
    return (
      ...
      >
        ...
        <View style={styles.islandWaveform}>
          {[8, 13, 10, 14, 9].map((height, index) => (
...
function RoundIcon({
  ...
  color?: string;
  icon: "chevron-down" | "close" | "pause" | "play";
  onPress: () => void;
  size?: "small" | "medium";
}) {
