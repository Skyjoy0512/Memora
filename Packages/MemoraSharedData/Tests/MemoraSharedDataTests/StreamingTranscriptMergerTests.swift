...
@Suite("Streaming transcript merger")
struct StreamingTranscriptMergerTests {
    ...
    func mergerAddsChunkOffsetToSegmentTimes() {
        ...
        let result = TranscriptionResult(fullText: "こんにちは", language: "ja", segments: [
    ...
    func mergerPreservesEstimatedTiming() {
        ...
        let result = TranscriptionResult(fullText: "こんにちは", language: "ja", segments: [
            TranscriptionSegment(id: "s1", speakerLabel: "", startSec: 1.0, endSec: 3.0, text: "こんにちは", isEstimatedTiming: true)
        ])
        merger.append(chunk: chunk, result: result)
    ...
    func mergerJoinsTextWithNewlines() {
        var merger = StreamingTranscriptMerger()
        merger.append(chunk: AudioChunk(index: 0, startSec: 0, endSec: 90, url: URL(fileURLWithPath: "/tmp/a.m4a"), isTemporary: true), result: TranscriptionResult(fullText: "hello", language: "ja", segments: []))
        merger.append(chunk: AudioChunk(index: 1, startSec: 90, endSec: 180, url: URL(fileURLWithPath: "/tmp/b.m4a"), isTemporary: true), result: TranscriptionResult(fullText: "world", language: "ja", segments: []))
        #expect(merger.finalize().fullText == "hello\nworld")
    }

    @Test("最初の結果から言語が採用される")
    func mergerUsesFirstResultLanguage() {
        var merger = StreamingTranscriptMerger()
        merger.append(chunk: AudioChunk(index: 0, startSec: 0, endSec: 90, url: URL(fileURLWithPath: "/tmp/a.m4a"), isTemporary: true), result: TranscriptionResult(fullText: "a", language: "en", segments: []))
        merger.append(chunk: AudioChunk(index: 1, startSec: 90, endSec: 180, url: URL(fileURLWithPath: "/tmp/b.m4a"), isTemporary: true), result: TranscriptionResult(fullText: "b", language: "ja", segments: []))
    ...
    func preferredLanguageOverridesDetected() {
        var merger = StreamingTranscriptMerger()
        merger.append(chunk: AudioChunk(index: 0, startSec: 0, endSec: 90, url: URL(fileURLWithPath: "/tmp/a.m4a"), isTemporary: true), result: TranscriptionResult(fullText: "a", language: "en", segments: []))
        #expect(merger.finalize(preferredLanguage: "ja-JP").language == "ja")
    }
}
