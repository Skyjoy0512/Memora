...
### 0.1 エージェント役割（2026-08）

- **Sol**: Codex上で開発計画、設計、ADR、受け入れ条件を所有する。
- **Luna**: Codex上のGPT-5.6 Lunaとして、Solが確定したタスクの主実装を担当する。
- **OpenCode**: DeepSeekモデルだけを使用し、独立レーンの実装、機械的修正、検証を担当する。
- **Claude**: 定期的なread-onlyレビューと設計見直し提案を担当する。

詳細は`docs/agent-operating-model.md`を正本とする。Claudeの提案を採用して設計正本を更新する責任はSolにあり、Luna / OpenCodeは設計を推測で変更しない。
