...
| HeroUI をラップする | Memora の情報構造を固定するため | 8 |
...
- **責務**: Bottom Tab + 中央録音 CTA
- **理由**: Expo Router の `tabBar` レンダラ。HeroUI の `Tabs` は用途が異なる
- **基盤**: `Surface` + `PressableFeedback` + `Text`
- **props**: Expo Router の `BottomTabBarProps`
- **states**: 通常 / 録音中 / 処理中
- **a11y**: `accessibilityRole="tab"` + `accessibilityState={{ selected }}`。中央ボタンは `button`
- **使用画面**: `(tabs)/_layout.tsx`
