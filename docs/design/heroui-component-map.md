...
| UI 用途 | HeroUI Native | カスタム | 理由 |
|---|---|---|---|
| Bottom Tab バー | 該当なし | **必要** | Expo Router の `tabBar` レンダラ。内部は `Surface` `PressableFeedback` `Text` で構成 |
| 中央録音ボタン | `Button` をラップ | **一部必要** | 状態（待機/録音中/一時停止）で表示が変わるため |
| グローバル録音バー | `Surface` をラップ | **一部必要** | 経過時間と操作を持つ Memora 固有 UI |
| 画面内タブ切替 | `Tabs` | 不要 | `.List` `.Trigger` `.Indicator` を使用 |
| ヘッダー | Expo Router | 不要 | ネイティブヘッダーを使用 |
...
| 現行 | 新設計での扱い |
|---|---|
| `FileCard` | `MeetingCard` へ改名。File 概念廃止に伴う |
