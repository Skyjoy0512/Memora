...
- Current branch observed when this doc was created: `feat/v6-10-polish`.
- Important: there were pre-existing uncommitted changes when this doc was created. Do not overwrite or revert unrelated work.
...
- Blockers: simulator device creation is currently blocked by CoreSimulator storage permissions. `xcrun simctl create "Memora RN iPhone 17 Pro" ... iOS-26-5` fails with `Device was allocated but was stuck in creation state`; CoreSimulator.log reports `Operation not permitted` writing under `/Volumes/HIKSEMI/Xcode/CoreSimulator/Devices/...`.
...
- Blockers: live iOS simulator UI inspection is still blocked by the local CoreSimulator storage permission issue under `/Volumes/HIKSEMI/Xcode/CoreSimulator/Devices`.
...
- Blockers: live simulator UI remains blocked by the existing CoreSimulator storage/permission issue under `/Volumes/HIKSEMI/Xcode/CoreSimulator/Devices`.
...
- CoreSimulator investigation: retried `xcrun simctl create` and it still fails with `Device was allocated but was stuck in creation state`. Traced the underlying `CoreSimulator.log` error to `Error copying sample content to path /Volumes/HIKSEMI/Xcode/CoreSimulator/Devices/<uuid>/data ... NSPOSIXErrorDomain Code=1 "Operation not permitted"`.
- Root cause found: `~/Library/Developer/CoreSimulator` is a symlink to `/Volumes/HIKSEMI/Xcode/CoreSimulator` (added 2026-07-08, presumably to work around the internal disk being at 92% capacity with only ~17GB free). The external volume was also mounted with the `noowners` flag, which was fixed this session (`sudo diskutil enableOwnership /Volumes/HIKSEMI`, confirmed via `mount | grep HIKSEMI` no longer showing `noowners`). Full Disk Access was also granted to `/Applications/Xcode.app`. Neither fix resolved the sample-content-copy `EPERM`; the identical error persisted after each fix and after restarting `CoreSimulatorService`. Basic file writes/mkdir to the same directory succeed fine from a normal shell, which rules out a plain Unix permission problem and points to `CoreSimulatorService` itself being unable to complete its content copy (likely an ACL/xattr/SIP-metadata propagation limitation specific to copying the runtime's protected sample-content payload onto a non-primary-volume-group external APFS disk) — this was not fully root-caused beyond that point.
...
- Follow-up same session: manually replicated CoreSimulator's sample-content copy with `ditto <runtime SampleContent> /Volumes/HIKSEMI/.../Devices/testcopy123/data` as the current user and it succeeded (exit 0), proving the filesystem/ownership fix from earlier in this session is fully correct. This isolated the remaining failure to `CoreSimulatorService`'s own live Full Disk Access authorization state.
...
- Created simulator `Memora RN Test` (iPhone 17 Pro, iOS 26.5), UDID `458A23AB-B4A3-43BF-8F40-4D6F56903088`, device data stored on `/Volumes/HIKSEMI` as intended.
...
- Decisions: Xcode 標準 DerivedData はすでに `/Volumes/HIKSEMI/Xcode/DerivedData` への symlink で外部化済みだった。今回不足していた QA スクリプトと XcodeBuildMCP の固有 DerivedData だけを追加で外部化した。Simulator runtime／Apple 管理キャッシュは無理に移動しない。
