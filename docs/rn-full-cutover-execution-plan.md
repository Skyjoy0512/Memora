...
| プロジェクト move | 実装 | **配線済み（2026-08-10）**: `listProjects` bridge（Project を title 昇順、`[{id, title}]`）+ `moveAudioFile` bridge。FileDetail「プロジェクトに移動」はプロジェクト選択シート（先頭「Inbox（個人）」）から移動してファイル更新。Tasks は `task.projectId` → プロジェクト名表示（該当なし/未設定は「個人タスク」） | 実機 QA | G / F | プロジェクト移動が永続化 |
| タスク tasks | 実装 | **#180 で実データ化済み**: `TodoItem` SwiftData + `MemoraSharedStoreTaskBridgeAdapter` + TasksScreen CRUD。**タスク化導線も配線済み（2026-08-10）**: FileDetail 文字起こし segment / AskAI 回答 → `createTask`（`src/native/taskLogic.ts`） | 日付選択・概要タブ「次のアクション → タスク」・タスクのプロジェクト選択（いずれも推奨/スコープ判断） | C / F | タスクの実データ契約 |
